...
class SyncProject(models.Model):
    ...
    def _get_eval_context(self, job, log):
        ...
        base_trigger_name = job.parent_job_id.trigger_name if job.parent_job_id else job.trigger_name
        eval_context = dict(
            **link_functions,
            ...
            **{
                ...
                "user": self.env.user,
                "task": job.task_id,
                "base_trigger": base_trigger_name,
        ...
        try:
            ...
            raise ValidationError(_('Common Code Error\n') + str(e))

        executing_common_code = time.time() - start_time
        log(
            "[Code Statics] Reading project data: %05.3f sec; Preparing custom evaluation context: %05.3f sec; Executing common code: %05.3f sec"
