...
retry = Retry(total=3, connect=2, backoff_factor=0.3, status_forcelist=(500, 502, 504, 508))
...
class SyncProjectDemo(models.Model):
    ...
    @api.model
    def _eval_context_http_endpoint(self, secrets, eval_context):
        """
        Additional functions for HTTP connections:

        * http_request(GETOPTIONS|HEAD|POST|PUT|PATCH|DELETE, *args, **kwargs)

        Auto-setted for each request:
            url: params.URL
            timeout: params.TIMEOUT [optional]
            auth: secrets.USER & secrets.PASS [optional]

        See requests docs: https://docs.python-requests.org/en/master/user/quickstart/
        """
        log = eval_context["log"]
        params = eval_context["params"]
        log_transmission = eval_context["log_transmission"]
        enable_auth = bool(params.get("USER") and params.get("PASS"))
        auth_param = HTTPBasicAuth(secrets.USER, secrets.PASS) if enable_auth else None

        if not params.get("URL"):
            raise UserError(_("HTTP/S URL not setted"))

        def http_request(rm='GET', *args, **kwargs):

            log_transmission("HTTP %s REQUEST" % rm, json.dumps([args, kwargs]))
            ...
            try:
                ...
                raise ValidationError('%s %s %s' % (rm, params.URL, err.args[0].reason))
            except requests.exceptions.Timeout:
                raise RetryableJobError("TIMEOUT REACHED")
            except requests.exceptions.HTTPError as err:
                raise RetryableJobError("HTTP ERROR: %s" % err)
            return res
